...
public Q_SLOTS:
    ...
    void onFilterChanged(const QString &name, bool state);
...
struct AdBlockRuleConfig {
    QString name;
    bool enabled;

    AdBlockRuleConfig(const QString &name, bool enabled) : 
        name(name), enabled(enabled) {}

    QString filePath(const QString &adblockDirectory) {
        return adblockDirectory + name + ".txt";
    }

    QString filePathQRC() {
      return ":assets/adblock/rules/" + name + ".txt";
    }
};
